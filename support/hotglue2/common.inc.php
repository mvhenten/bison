<?php
/**
 * support/hotglue2/common.inc.php
 *
 ...
 *
 * @param string  $category cache category (e.g. 'page')
 * @param string  $name     (optional) item name (optional)
 */
 ...
 * @param bool    $no_header (optional) don't output any header
...
function is_auth() {
    ...
    if ( isset($request_uri_parts['path']) && isset($_SESSION['uniqid']) ) {
        $uid = $_SESSION['uniqid'];

        $path_parts = split( '/', $request_uri_parts['path'] );

        return in_array( $uid, $path_parts );
    }
 ...
 *
 * @param unknown $s object (e.g. page.rev.obj)
 * @return bool
 */
 ...
 *
 * @param bool    $header_only (optional) only send header information
 */
 ...
 * @param string  $name (optional) current object name (when applicable)
 ...
 * @param string  $orig_fn  (optional) the original filename on the client machine (optional)
